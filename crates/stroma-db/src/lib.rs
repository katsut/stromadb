...
impl Db {
    ...
    fn neighborhood(&self, req: &Value, snap: &Snapshot) -> DbResult<Value> {
        ...
        let labels = req["allowed_labels"].as_u64().map(|m| m as u32).unwrap_or(u32::MAX);
        let pred = match req["predicate"].as_str() {
            Some(p) => Some(self.cat.field_id(p).ok_or(format!("unknown predicate: {p}"))?),
            None => None,
        };
        let visible = |n: u64| self.cat.node_label(n).is_none_or(|l| (labels >> l) & 1 == 1);
        ...
        let nodes: Vec<Value> = depth.iter().map(|(&id, &d)| json!({ "id": id, "depth": d })).collect();
